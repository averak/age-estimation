...
class Analyzer(BaseAnalyzer):
    ...
    def export_heatmap(self, train_humans: list, test_humans: list, train_results: np.ndarray, test_results: np.ndarray):
        ...
        plt.hist2d(θ_pred_list_train, θ_true_list_train, bins=116, range=[(0, 116), (0, 116)], cmin=1)
        plt.colorbar()
        plt.xlabel("θ")
        plt.ylabel("Age")
        plt.xlim(0, 116)
        ...
        ax = figure.add_subplot(111)
        ax.set_aspect('equal', adjustable='box')
        plt.hist2d(θ_pred_list_test, θ_true_list_test, bins=116, range=[(0, 116), (0, 116)], cmin=1)
        plt.colorbar()
        plt.xlabel("θ")
        plt.ylabel("Age")
        plt.xlim(0, 116)
